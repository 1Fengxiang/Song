...
<!-- 
    <div class="song-total-box" v-for="outerIndex in 6" :key="outerIndex">
      <div v-for="index in 4" :key="index" :class="outerIndex === 2 ? 'new-class' : 'song-img'">
        <router-link  style="text-decoration: none !important;"  v-if="1" :to="{ path: '/api/songList', query: { id: 1 } }">
          <a  target="_self">
          <el-image class="everysongs"
           v-if="1"
            src="https://tse3-mm.cn.bing.net/th/id/OIP-C.30ocMaZa09CsGObHujVGSgHaEK?w=310&h=180&c=7&r=0&o=5&dpr=1.6&pid=1.7"
            fit="cover"
            style="width: 160px; height: 160px">
          </el-image>
          
        </a>
        <el-link  v-if="1"  target="_blank" style="font-size: 14px;" class="custom-link">你好</el-link>
        </router-link>
        
      </div>
    </div> -->

    <div  class="song-total-box" v-for="outerIndex in Math.ceil(paginatedData.length/4)" :key="outerIndex">
      <div v-for="index in 4" :key="index" :class="outerIndex === 2 ? 'new-class' : 'song-img'">
        <router-link  style="text-decoration: none !important;"  v-if="songList[(currentPage - 1) * pageSize+(outerIndex-1)*4+index-1]" :to="{ path: '/api/songList', query: { id: songList[(currentPage - 1) * pageSize+(outerIndex-1)*4+index-1].listId } }">
          <a  target="_self">
          <el-image class="everysongs"
           v-if="songList[ (currentPage - 1) * pageSize+(outerIndex-1)*4+index-1]"
            :src="songList[(currentPage - 1) * pageSize+(outerIndex-1)*4+index-1].listImg"
            fit="cover"
            style="width: 160px; height: 160px">
          </el-image>
          
        </a>
        <el-link  v-if="songList[(currentPage - 1) * pageSize+(outerIndex-1)*4+index-1]"  target="_blank" style="font-size: 14px;" class="custom-link">{{songList[(currentPage - 1) * pageSize+(outerIndex-1)*4+index-1].listTitle}}</el-link>
        </router-link>
    ...
    <div>
      ...
      :total="total"
      style="margin:20px 0px 0px 350px;"
...
// 最大高度
  ...
  if(this.data)
  {
    ...
    paginatedData() {
      ...
      return this.songList.slice(start, start + this.pageSize);
    },
  },
  beforeDestroy() {
