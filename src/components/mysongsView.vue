<template>
  ...
  <div class="container">
    ...
    <el-menu
      ...
      <el-submenu index="1" >
        ...
        <el-menu-item-group v-for="index in 1" :key="index">
          <el-menu-item :index="'1-' + index">
          <img :src="data.headshot" alt="无法显示" class="musicImg">
        <span>我喜欢的音乐</span>
          </el-menu-item>
      ...
      <el-submenu index="2" :default-opened="true">
        <template slot="title">
          <i class="el-icon-star-on"></i>
          <span>我收藏的歌单({{musiclist.length}})</span>
        </template>
        <el-menu-item-group v-for="index in musiclist.length" :key="index">
          <router-link  style="text-decoration: none !important;"  v-if="musiclist[index-1]" :to="{ path: '/api/songList', query: { id: musiclist[index-1].listId } }">
          <el-menu-item :index="'2-' + index">  
            <img v-if="musiclist[index-1]" :src="musiclist[index-1].listImg" alt="无法显示" class="musicImg">
            <span v-if="musiclist[index-1]">{{musiclist[index-1].listTitle}}</span>
          </el-menu-item>
          </router-link>
        </el-menu-item-group>
      </el-submenu>
  ...
  <img :src="data.headshot" alt="无法显示" class="songListImg">

<div class="text-container">
  <div class="spans-container">
    
    <h1 class="title">我喜欢的音乐</h1>
    <span style="font-size: 15px; color: blue;">风闲<span style="padding-left: 100px; color: gray;">2023-11-1创建</span></span>
    <div>
      <a> 
...
getActive(songId)
   {   
    ...
    const promises = this.songList.map(song => {
  ...
  // 最大高度
    ...
    return this.arr.length-1;
      ...
      closeDrawer(){  
       ...
       lyc()  //点击播放图片后展示歌词
       {
         ...
         this.getLyric(JSON.parse(localStorage.getItem('Nowsong')).songLyc);
      ...
      }).then((result)=>
    {
      ...
      axios.get("http://192.168.3.226:1111/api/musics",{
        params:{
      ...
      axios.get("http://192.168.3.226:1111/api/musiclist",{
        params:{
          userId:this.data.userId
        }
      }).then((result)=>{
