<template>
  ...
  <div class="footer-box">
    ...
    <div class="box-top"> 
      <el-breadcrumb separator="|">
        <el-breadcrumb-item  style="font-size: 20px;"><a >热门推荐</a></el-breadcrumb-item>
        <el-breadcrumb-item :to="{ path: '/api/sort', query: { type: '华语' } }">
      <a>华语</a>
    </el-breadcrumb-item>
    <el-breadcrumb-item :to="{ path: '/api/sort', query: { type: '欧美' } }">
      <a>欧美</a>
      </el-breadcrumb-item>
      <el-breadcrumb-item :to="{ path: '/api/sort', query: { type: '流行' } }">
        <a>流行</a>
        </el-breadcrumb-item>
    ...
    <div class="song-total-box" v-for="outerIndex in songList.length>4?2:1" :key="outerIndex">
      <div v-for="index in 4" :key="index" :class="outerIndex === 2 ? 'new-class' : 'song-img'">
        <router-link  style="text-decoration: none !important;"  v-if="songList[(outerIndex-1)*4+index-1]" :to="{ path: '/api/songList', query: { id: songList[(outerIndex-1)*4+index-1].listId } }">
          <a  target="_self">
          <el-image class="everysongs"
           v-if="songList[(outerIndex-1)*4+index-1]"
            :src="songList[(outerIndex-1)*4+index-1].listImg"
            fit="cover"
            style="width: 160px; height: 160px">
          </el-image>
          
        </a>
        <el-link  v-if="songList[(outerIndex-1)*4+index-1]"  target="_blank" style="font-size: 14px;" class="custom-link">{{songList[(outerIndex-1)*4+index-1].listTitle}}</el-link>
        </router-link>
  ...
  <div>
    <div class="personal-box">
      <img :src="data.headshot" alt="头像"  class="personal_img"  style="cursor: pointer;"  v-if="token"/>
      
      <div class="text-container">
    <span id="animated-text"></span>
    <div class="countdown" v-if="isLoggedIn">距离生日还有 {{ countdown }} 天</div>
    </div>
    </div>

    
    <div class="clock-box">
      <h2><i class="el-icon-alarm-clock"></i>电子时钟</h2>
      <div style="font-size: 24px;">{{ time }}</div>
      <div style="font-size: 18px;">{{ formattedDate }} 星期{{ dayOfWeek }}</div>
    </div>

    <div class="song-box">
    <h2 style="text-align: center;">热歌榜</h2>
    <div v-for="(song, index) in this.allsongs" :key="index" class="song-card">
      <el-button icon="el-icon-video-play" circle @click="playSongTemp(song.songFilepath,index,1)"></el-button>
      <a  class="song-title">{{ song.songName }}</a>
    </div>
    <!-- 可以继续添加更多歌曲 -->
</div>
...
export default {
    ...
    "https://pic2.zhimg.com/v2-3563e6cde64c2dee537ebc413ed5029c_1200x500.jpg"
    ],
    ...
    stopUpdateTimer() {
         ...
         }).then((result)=>
      {
        ...
        if(result.data.data==='true')
          this.$store.state.haveAnymsg=true;
...
let currentSentenceIndex = -1; // 当前句子的索引
const text = document.getElementById("animated-text");
let usedIndexes = []; // 存储已使用的索引

// 初始化打字效果
const typeText = (originalText) => {
  ...
  const type = () => {
    if (index < originalText.length) {
      ...
      index++;
      setTimeout(type, 240); // 设置每个字符出现的时间间隔
    } else {
      // 一次打字结束，开始删除
      setTimeout(deleteText, 1000); // 等待一段时间后开始删除
    }
  };

  const deleteText = () => {
    if (index > 0) {
      text.innerHTML = originalText.substring(0, index - 1);
      index--;
      setTimeout(deleteText, 50); // 设置每个字符删除的时间间隔
    } else {
      // 删除完成后，准备下一个句子
      setNextSentence();
      setTimeout(() => {
        typeText(sentences[currentSentenceIndex]); // 继续打字下一个句子
      }, 500); // 等待一段时间后重新开始
    }
  };
...
const setNextSentence = () => {
  ...
  usedIndexes.push(randomIndex); // 记录已使用的索引
  currentSentenceIndex = randomIndex; // 更新当前句子的索引
};
